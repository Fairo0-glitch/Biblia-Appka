    ...
    <div className="min-h-screen bg-[#0f172a] text-slate-200 font-sans">
      <header className="relative pt-20 pb-32 px-4 overflow-hidden rounded-b-[4rem] bg-slate-900 shadow-2xl text-center">
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          ...
          {loading ? <div className="h-40 flex items-center justify-center italic text-slate-500">Otwieranie...</div> : currentVerse ? (
            <div className="space-y-10">
              <h1 className="text-4xl md:text-6xl font-serif italic text-white leading-tight px-4">"{currentVerse.verse_text}"</h1>
          ...
          ) : <div className="py-20 text-slate-500">Brak Słowa na ten dzień.</div>}
        </div>
      </header>
      <main className="max-w-6xl mx-auto px-4 -mt-16 relative z-20 pb-20 grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-8 bg-slate-800/40 backdrop-blur-2xl p-8 md:p-12 rounded-[3rem] border border-white/5 shadow-2xl">
          <h4 className="text-2xl font-bold text-white mb-10 flex justify-between">Refleksje <span className="text-xs bg-white/5 px-4 py-2 rounded-full text-amber-500">{comments.length} WPISÓW</span></h4>
          ...
          <div className="space-y-4 pt-8 border-t border-white/10">
            ...
            <textarea placeholder="Co mówi do Ciebie to Słowo?..." value={newComment} onChange={(e) => setNewComment(e.target.value)} className="w-full p-6 rounded-3xl bg-slate-900 border border-white/10 text-white outline-none focus:border-amber-500 min-h-[120px] resize-none" />
            <button onClick={handleAddComment} className="w-full py-5 bg-amber-600 hover:bg-amber-500 text-white font-black rounded-2xl transition-all uppercase tracking-widest text-xs">Udostępnij</button>
          </div>
        </div>
      </main>
    </div>
