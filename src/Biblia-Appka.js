...
function App() {
  ...
  // DYNAMICZNY WYGLĄD NA PODSTAWIE RANGI
  const getTheme = () => {
    if (streak >= 365) return { bg: "bg-amber-950", accent: "text-amber-400", border: "border-amber-500/30", aura: "bg-amber-500/20", btn: "bg-amber-600" };
    if (streak >= 200) return { bg: "bg-red-950", accent: "text-red-400", border: "border-red-500/30", aura: "bg-red-500/20", btn: "bg-red-700" };
    if (streak >= 100) return { bg: "bg-emerald-950", accent: "text-emerald-400", border: "border-emerald-500/30", aura: "bg-emerald-500/20", btn: "bg-emerald-700" };
    if (streak >= 30) return { bg: "bg-purple-950", accent: "text-purple-400", border: "border-purple-500/30", aura: "bg-purple-500/20", btn: "bg-purple-700" };
    return { bg: "bg-slate-950", accent: "text-blue-400", border: "border-blue-500/30", aura: "bg-blue-500/10", btn: "bg-blue-700" };
  };
    ...
    <div className={`min-h-screen ${theme.bg} text-slate-200 font-sans pb-20 overflow-x-hidden relative transition-colors duration-1000`}>
      ...
      <header className="relative pt-12 pb-28 px-4 text-center z-10">
        <div className="max-w-2xl mx-auto">
          <div className="flex flex-col items-center mb-10 gap-2">
             <div className="group relative">
               ...
               <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-64 bg-slate-950 border border-white/10 rounded-3xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 p-4 text-left">
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-500 mb-3 border-b border-white/5 pb-1 uppercase">Postęp Roku 2026</p>
                  {nextBadge && (
                    <div className="mb-4">
                      ...
                      <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden">
                        <div className={`h-full ${theme.btn.replace('bg-', 'bg-')}`} style={{ width: `${(streak/nextBadge.day)*100}%` }}></div>
                      </div>
                    </div>
                  )}
          ...
          <span className={`px-5 py-1.5 rounded-full bg-white/5 border ${theme.border} ${theme.accent} text-[10px] font-black uppercase tracking-[0.4em] mb-10 inline-block`}>Słowo Życia na {selectedDate}</span>
          
          {loading ? (
            <div className="py-20 animate-pulse opacity-50 italic">Wczytywanie...</div>
          ) : currentVerse ? (
            <div className="flex flex-col items-center animate-fadeIn">
              {currentVerse.audio_url && (
                <div className="w-full max-w-sm mb-12">
                   <p className="text-[10px] uppercase font-black opacity-40 tracking-[0.2em] mb-4 uppercase">Audio</p>
              ...
              <div className="space-y-6">
                ...
                <cite className={`text-lg font-bold ${theme.accent} block uppercase tracking-widest opacity-90">— {currentVerse.reference}</cite>
              </div>
            </div>
          ) : (
            <div className="py-20 opacity-50">Wybierz inną datę...</div>
          )}
        </div>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-4 -mt-10 relative z-20 space-y-6">
        <section className="bg-black/20 backdrop-blur-3xl p-6 rounded-[2.5rem] border border-white/5 shadow-2xl">
          <h3 className="text-[10px] font-black mb-4 uppercase tracking-widest opacity-40">📅 Wybierz dzień</h3>
        ...
        <section className="bg-black/10 backdrop-blur-2xl p-6 md:p-10 rounded-[3rem] border border-white/5">
          ...
          <div className="space-y-4 pt-6 border-t border-white/5">
            <input type="text" placeholder="Imię..." value={author} onChange={(e) => setAuthor(e.target.value)} className="w-full p-4 rounded-2xl bg-black/40 border border-white/5 text-white outline-none focus:border-white/20" />
